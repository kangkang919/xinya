...
> **核心原则：** 调试问题时，禁止猜测原因直接改代码。必须用可观测手段逐层验证假设，找到确切的不一致点后再修复。

**四层验证模型：**

| 层级 | 验证内容 | 示例命令 |
|------|---------|----------|
| 数据层 | 原始数据是否正确 | `localStorage.getItem('key')` |
| 逻辑层 | 数据处理逻辑是否正确 | `THEME_BG[t]` 映射返回值 |
| DOM 层 | 实际渲染结果 | `getComputedStyle(el).backgroundColor` |
| 事件层 | 交互触发是否生效 | 手动 `dispatchEvent` 后状态变化 |

**关键规则：**
- 每一步都要有具体的验证命令和预期值
- 只有当某一步的实际值 ≠ 预期值时，才定位到该层修复
- 连续修复 2 次无效时，必须停止当前方向，回到第 1 层重新验证
...
| 2026-07-09 | 心得详情页返回逻辑优化（from 参数区分萌芽/AI弹窗/枝叶来源） | 已验收 |
| 2026-07-09 | 主题精简为 2 套（春日萌芽 + 墨色幽微），默认春日萌芽 | 已验收 |
| 2026-07-09 | 暗色主题 F5 刷新失效修复（SSR hydration 问题，改用纯客户端 useEffect） | 已验收 |
| 2026-07-09 | 根系页导出功能恢复（lib/export-utils.ts + root/page.tsx） | 已验收 |
| 2026-07-09 | 心得详情页「编辑此篇」改为「分享此篇」（html2canvas 截图 + 分享面板） | 已验收 |
| 2026-07-09 | 截图字体统一（截图内容使用统一 font-family） | 已验收 |
...
| F9 拾遗 | 截图字体统一（截图内容使用统一 font-family） | 已验收 |
