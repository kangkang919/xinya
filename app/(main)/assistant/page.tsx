...
function ChatView({ profile, isDark, cardBg, cardBorder, titleColor, dimColor, inputBg, inputBorder }: ChatProps) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      {/* ===== 顶栏（§9.4：中间 flex-1+min-width-0 省略号，两侧 flex-shrink:0） ===== */}
      <header
        className="flex items-center gap-2 px-4 py-2.5 flex-shrink-0"
        style={{ background: cardBg, borderBottom: `1px solid ${cardBorder}` }}
      >
      ...
      >
        <input
          className="flex-1 min-w-0 rounded-full px-4 py-2.5 text-[14px] outline-none transition-colors disabled:opacity-60"
