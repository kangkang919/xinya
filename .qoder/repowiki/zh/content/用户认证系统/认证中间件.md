# 认证中间件

<cite>
**本文引用的文件**   
- [middleware.ts](file://middleware.ts)
- [lib/auth.ts](file://lib/auth.ts)
- [app/api/auth/login/route.ts](file://app/api/auth/login/route.ts)
- [app/api/auth/register/route.ts](file://app/api/auth/register/route.ts)
- [app/api/auth/verify-email/route.ts](file://app/api/auth/verify-email/route.ts)
- [app/api/auth/forgot-password/route.ts](file://app/api/auth/forgot-password/route.ts)
- [app/api/auth/reset-password/route.ts](file://app/api/auth/reset-password/route.ts)
- [app/api/auth/set-password/route.ts](file://app/api/auth/set-password/route.ts)
- [app/page.tsx](file://app/page.tsx)
</cite>

## 更新摘要
**变更内容**   
- 增强了认证中间件的已登录用户重定向机制，防止已登录用户访问登录/注册页面
- 添加了详细的调试日志记录功能，包括路径名、认证状态、令牌存在性和令牌预览信息
- 优化了故障排查能力，提供更清晰的中间件执行流程追踪

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件为心芽应用的认证中间件系统提供完整技术文档，重点覆盖：
- Next.js 中间件的实现原理与请求拦截机制
- 路由级权限控制（公开、受保护、管理员）的配置与扩展方案
- 会话状态维护与跨请求用户上下文传递
- 未认证用户的重定向逻辑与访问控制策略
- **新增**：已登录用户访问认证页面的智能重定向机制
- **新增**：增强的调试日志记录功能，支持详细的认证状态追踪
- 中间件性能优化建议与调试方法
- 安全头设置与 CORS 配置建议

## 项目结构
本项目采用 Next.js App Router。认证相关的关键位置如下：
- 根级中间件：用于页面路由的鉴权拦截与重定向，包含增强的调试日志功能
- 认证工具库：JWT 签发/校验、Cookie 配置、当前用户解析
- API 路由：登录、注册、邮箱验证、密码重置等流程
- 首页入口：未登录时重定向到登录页

```mermaid
graph TB
MW["中间件<br/>middleware.ts<br/>增强版"] --> Pages["页面路由<br/>app/(auth), app/(main) 等"]
MW --> |跳过| API["API 路由<br/>app/api/*"]
MW --> |跳过| Static["静态资源<br/>_next/*, icons, *.ext"]
Pages --> AuthLib["认证库<br/>lib/auth.ts"]
API --> AuthLib
AuthLib --> Cookies["Cookie: xinya_token"]
MW --> DebugLog["调试日志<br/>路径名/认证状态/令牌预览"]
```

**图表来源**
- [middleware.ts:1-37](file://middleware.ts#L1-L37)
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)

**章节来源**
- [middleware.ts:1-37](file://middleware.ts#L1-L37)
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)

## 核心组件
- 中间件（middleware.ts）
  - 作用：在请求到达页面之前进行鉴权判断；对静态资源和 API 直接放行；对未登录用户重定向至登录页
  - **新增功能**：已登录用户访问认证页面时自动重定向到首页
  - **新增功能**：详细的调试日志记录，包含路径名、认证状态、令牌存在性和令牌预览信息
  - 关键行为：
    - 匹配规则：排除静态资源与图标等路径
    - 白名单：认证相关页面允许无 Token 访问
    - 智能重定向：已登录用户访问 /login、/register 等页面时重定向到 /
    - 鉴权：检查 Cookie 中是否存在 xinya_token
    - 重定向：未登录则跳转到 /login
- 认证库（lib/auth.ts）
  - JWT 签发与校验：使用环境变量或默认密钥，有效期 30 天
  - Cookie 配置：名称、httpOnly、sameSite、maxAge、path
  - 当前用户解析：从 Cookie 读取 token 并解析出 userId
- 认证 API 路由
  - 登录：校验凭据、签发 JWT、写入 Cookie，包含调试日志
  - 注册：校验参数、创建用户、发送验证码
  - 邮箱验证：校验验证码、标记已验证、自动登录
  - 忘记密码/重置密码：生成一次性令牌、发送邮件、更新密码
  - 设置密码：需登录态，更新密码

**章节来源**
- [middleware.ts:1-37](file://middleware.ts#L1-L37)
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)
- [app/api/auth/login/route.ts:1-43](file://app/api/auth/login/route.ts#L1-L43)
- [app/api/auth/register/route.ts:1-56](file://app/api/auth/register/route.ts#L1-L56)
- [app/api/auth/verify-email/route.ts:1-38](file://app/api/auth/verify-email/route.ts#L1-L38)
- [app/api/auth/forgot-password/route.ts:1-34](file://app/api/auth/forgot-password/route.ts#L1-L34)
- [app/api/auth/reset-password/route.ts:1-31](file://app/api/auth/reset-password/route.ts#L1-L31)
- [app/api/auth/set-password/route.ts:1-27](file://app/api/auth/set-password/route.ts#L1-L27)

## 架构总览
下图展示了浏览器、Next.js 中间件、API 路由与认证库之间的交互关系，包含增强的调试日志功能。

```mermaid
sequenceDiagram
participant B as "浏览器"
participant M as "中间件<br/>middleware.ts<br/>增强版"
participant P as "页面/布局"
participant A as "API 路由"
participant L as "认证库<br/>lib/auth.ts"
B->>M : 请求任意页面
M->>M : 判断是否静态/API
alt 是静态或API
M-->>B : 放行
B->>A : 调用API(可选)
A->>L : 解析当前用户ID
L-->>A : 返回userId或null
A-->>B : 业务响应
else 非静态且非API
M->>M : 检查是否在认证白名单
M->>M : 记录调试日志<br/>路径名/认证状态/令牌预览
alt 在白名单
M->>M : 检查用户是否已登录
alt 已登录用户访问认证页面
M-->>B : 重定向到 /
else 未登录用户
M-->>P : 放行渲染
end
else 不在白名单
M->>M : 检查Cookie是否有xinya_token
alt 有Token
M-->>P : 放行渲染
else 无Token
M-->>B : 重定向到/login
end
end
end
```

**图表来源**
- [middleware.ts:1-37](file://middleware.ts#L1-L37)
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)
- [app/api/auth/login/route.ts:1-43](file://app/api/auth/login/route.ts#L1-L43)

## 详细组件分析

### 中间件与请求拦截机制
- 匹配范围
  - 通过 matcher 排除静态资源与图标，避免不必要的处理开销
- **增强版鉴权流程**
  - 若路径以 /api/ 或 /_next/ 开头，直接放行
  - 若路径属于认证白名单（如 /login、/register、/verify-email、/forgot-password、/reset-password、/onboard、/showcase），执行以下逻辑：
    - **新增**：记录详细的调试日志，包含路径名、是否为认证页面、令牌存在性和令牌预览
    - **新增**：如果已登录用户访问认证页面，自动重定向到首页
    - 否则放行渲染
  - 否则检查 Cookie 中的 xinya_token，不存在则重定向到 /login
- **新增调试日志功能**
  - 记录路径名（pathname）
  - 记录是否为认证页面（isAuthPage）
  - 记录令牌存在性（hasToken）
  - 记录令牌预览信息（tokenPreview，仅显示前20个字符）
- 注意事项
  - 当前仅基于"是否存在 token"做简单鉴权，未校验 token 有效性
  - 未区分管理员与普通用户，需要扩展
  - 调试日志可用于生产环境的问题排查

```mermaid
flowchart TD
Start(["进入中间件"]) --> CheckStatic["是否静态或_next?"]
CheckStatic --> |是| Pass1["放行"]
CheckStatic --> |否| CheckAuthPages["是否认证白名单?"]
CheckAuthPages --> |是| LogDebug["记录调试日志<br/>路径名/认证状态/令牌预览"]
LogDebug --> HasToken{"已登录用户?"}
HasToken --> |是| RedirectHome["重定向到 /"]
HasToken --> |否| Pass2["放行"]
CheckAuthPages --> |否| HasToken2{"Cookie包含xinya_token?"}
HasToken2 --> |否| LogNoToken["记录无token日志"]
LogNoToken --> RedirectLogin["重定向到/login"]
HasToken2 --> |是| Pass3["放行"]
Pass1 --> End(["结束"])
Pass2 --> End
Pass3 --> End
RedirectHome --> End
RedirectLogin --> End
```

**图表来源**
- [middleware.ts:1-37](file://middleware.ts#L1-L37)

**章节来源**
- [middleware.ts:1-37](file://middleware.ts#L1-L37)

### 会话状态与用户上下文传递
- 会话载体
  - Cookie 名称：xinya_token
  - 属性：httpOnly、sameSite=lax、maxAge=30天、path=/
- 用户上下文获取
  - getCurrentUserId() 从 Cookie 读取 token，调用 verifyToken 解析 payload，返回 userId 或 null
- 登录与自动登录
  - 登录成功后通过 response.cookies.set 写入 Cookie，包含调试日志
  - 邮箱验证通过后同样写入 Cookie，实现自动登录
- 登出
  - 删除 Cookie 即可退出登录

```mermaid
classDiagram
class AuthLib {
+hashPassword(password) Promise~string~
+verifyPassword(password, hash) Promise~boolean~
+signToken(userId) string
+verifyToken(token) {userId}|null
+getCurrentUserId() Promise~string|null~
+COOKIE_CONFIG
}
class Middleware {
+middleware(request) NextResponse
+config.matcher
+调试日志功能
+已登录用户重定向
}
class LoginRoute {
+POST(req) NextResponse
+调试日志记录
}
class VerifyEmailRoute {
+POST(req) NextResponse
}
Middleware --> AuthLib : "读取Cookie/鉴权"
LoginRoute --> AuthLib : "签发JWT/写Cookie"
VerifyEmailRoute --> AuthLib : "签发JWT/写Cookie"
```

**图表来源**
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)
- [middleware.ts:1-37](file://middleware.ts#L1-L37)
- [app/api/auth/login/route.ts:1-43](file://app/api/auth/login/route.ts#L1-L43)
- [app/api/auth/verify-email/route.ts:1-38](file://app/api/auth/verify-email/route.ts#L1-L38)

**章节来源**
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)
- [app/api/auth/login/route.ts:1-43](file://app/api/auth/login/route.ts#L1-L43)
- [app/api/auth/verify-email/route.ts:1-38](file://app/api/auth/verify-email/route.ts#L1-L38)

### 路由级权限控制（公开、受保护、管理员）
- 当前实现
  - 公开路由：认证白名单内的页面可直接访问
  - **增强**：已登录用户访问认证页面时自动重定向到首页，提升用户体验
  - 受保护路由：除白名单外的页面需要携带有效 token
  - 管理员路由：尚未实现，需要扩展
- 扩展建议
  - 在中间件中增加角色判断逻辑，例如根据 cookie 或后端接口返回的角色信息
  - 将管理员路由集中管理，并在中间件中进行额外校验
  - 对于 API 路由，建议在各自 route.ts 中结合 getCurrentUserId 进行二次校验

```mermaid
flowchart TD
Req["请求进入"] --> RoleCheck{"是否管理员路由?"}
RoleCheck --> |是| AdminCheck["校验管理员角色"]
AdminCheck --> |通过| Allow["放行"]
AdminCheck --> |失败| Deny["拒绝访问"]
RoleCheck --> |否| ProtectedCheck{"是否受保护路由?"}
ProtectedCheck --> |是| TokenCheck{"是否携带有效token?"}
TokenCheck --> |是| Allow
TokenCheck --> |否| RedirectLogin["重定向到登录"]
ProtectedCheck --> |否| AuthPageCheck{"是否认证页面?"}
AuthPageCheck --> |是| LoggedInCheck{"用户是否已登录?"}
LoggedInCheck --> |是| RedirectHome["重定向到首页"]
LoggedInCheck --> |否| Allow
AuthPageCheck --> |否| Allow
```

**章节来源**
- [middleware.ts:1-37](file://middleware.ts#L1-L37)

### 未认证用户重定向与访问控制策略
- **增强版重定向逻辑**
  - 当访问受保护页面且 Cookie 不含 xinya_token 时，中间件会重定向到 /login
  - **新增**：当已登录用户访问认证页面（/login、/register 等）时，自动重定向到首页
- 首页入口
  - 根页面默认重定向到 /login，确保首次访问也进入登录流程
- 访问控制策略
  - 当前策略为"存在 token 即放行"，未校验 token 签名与过期时间
  - 建议改为严格校验 token 有效性，并结合服务端角色信息进行细粒度控制

**章节来源**
- [middleware.ts:1-37](file://middleware.ts#L1-L37)
- [app/page.tsx:1-5](file://app/page.tsx#L1-L5)

### 认证流程时序（登录、注册、邮箱验证、密码重置）
```mermaid
sequenceDiagram
participant U as "用户"
participant FE as "前端"
participant API as "API 路由"
participant DB as "数据库"
participant AUTH as "认证库"
U->>FE : 输入邮箱/密码
FE->>API : POST /api/auth/login
API->>DB : 查询用户
DB-->>API : 用户数据
API->>AUTH : verifyPassword()
AUTH-->>API : 结果
API->>AUTH : signToken()
AUTH-->>API : JWT
API->>API : 写入Cookie(xinya_token)
API-->>FE : 登录成功
U->>FE : 提交验证码
FE->>API : POST /api/auth/verify-email
API->>DB : 校验验证码
DB-->>API : 结果
API->>AUTH : signToken()
AUTH-->>API : JWT
API->>API : 写入Cookie(xinya_token)
API-->>FE : 验证成功并自动登录
Note over FE,MW : 中间件检测到已登录用户访问认证页面<br/>自动重定向到首页
```

**图表来源**
- [app/api/auth/login/route.ts:1-43](file://app/api/auth/login/route.ts#L1-L43)
- [app/api/auth/verify-email/route.ts:1-38](file://app/api/auth/verify-email/route.ts#L1-L38)
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)
- [middleware.ts:1-37](file://middleware.ts#L1-L37)

**章节来源**
- [app/api/auth/login/route.ts:1-43](file://app/api/auth/login/route.ts#L1-L43)
- [app/api/auth/register/route.ts:1-56](file://app/api/auth/register/route.ts#L1-L56)
- [app/api/auth/verify-email/route.ts:1-38](file://app/api/auth/verify-email/route.ts#L1-L38)
- [app/api/auth/forgot-password/route.ts:1-34](file://app/api/auth/forgot-password/route.ts#L1-L34)
- [app/api/auth/reset-password/route.ts:1-31](file://app/api/auth/reset-password/route.ts#L1-L31)
- [app/api/auth/set-password/route.ts:1-27](file://app/api/auth/set-password/route.ts#L1-L27)
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)

## 依赖关系分析
- 模块耦合
  - 中间件依赖 request.cookies 与 NextResponse
  - 认证库依赖 bcryptjs、jsonwebtoken、next/headers
  - API 路由依赖认证库与数据库客户端
- 外部依赖
  - jsonwebtoken：用于 JWT 签发与校验
  - bcryptjs：用于密码哈希与比对
  - next/headers：用于读写 Cookie
- 潜在风险
  - 中间件未校验 token 有效性，存在绕过风险
  - COOKIE_CONFIG.secure=false，生产环境应启用 HTTPS 并设置为 true

```mermaid
graph LR
MW["中间件<br/>增强版"] --> C["Cookie: xinya_token"]
MW --> DL["调试日志系统"]
API["API 路由"] --> AL["认证库"]
AL --> JWT["jsonwebtoken"]
AL --> BC["bcryptjs"]
AL --> H["next/headers"]
```

**图表来源**
- [middleware.ts:1-37](file://middleware.ts#L1-L37)
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)

**章节来源**
- [middleware.ts:1-37](file://middleware.ts#L1-L37)
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)

## 性能考虑
- 减少中间件处理范围
  - 利用 matcher 排除静态资源与图标，避免不必要计算
- 最小化鉴权开销
  - 当前仅检查 Cookie 是否存在，未进行网络 I/O，性能较好
  - 建议仅在必要时进行 token 校验，避免频繁解析
- **调试日志性能影响**
  - 调试日志使用 console.log，在生产环境中可能产生额外开销
  - 建议在生产环境禁用详细日志或使用条件日志记录
- 缓存与复用
  - 可在中间件层缓存常见白名单与路由策略，降低重复判断成本
- 日志与监控
  - 对重定向与鉴权失败进行轻量统计，便于定位热点路径

## 故障排查指南
- **新增调试日志功能**
  - 中间件现在记录详细的调试信息，包括：
    - 路径名（pathname）
    - 是否为认证页面（isAuthPage）
    - 令牌存在性（hasToken）
    - 令牌预览信息（tokenPreview，仅显示前20个字符）
  - 登录流程也包含详细的调试日志
- 常见问题
  - 登录后仍被重定向到登录页：检查 Cookie 是否正确写入且未被浏览器策略阻止
  - 已登录用户无法访问认证页面：确认重定向逻辑是否符合预期
  - 跨域问题：确认 SameSite 与 Secure 配置与部署环境一致
  - 验证码无效或过期：检查 emailToken 记录与过期时间
- **增强版调试步骤**
  - 查看服务器控制台输出，分析 [Middleware-DEBUG] 日志
  - 打开浏览器开发者工具，查看请求与响应头，确认 Cookie 是否携带
  - 在 API 路由中添加必要日志，打印解析到的 userId 与错误堆栈
  - 使用 curl 或 Postman 模拟请求，快速验证 API 行为
- 参考实现
  - 登录、注册、邮箱验证、密码重置等 API 均包含错误处理与日志输出

**章节来源**
- [middleware.ts:1-37](file://middleware.ts#L1-L37)
- [app/api/auth/login/route.ts:1-43](file://app/api/auth/login/route.ts#L1-L43)
- [app/api/auth/register/route.ts:1-56](file://app/api/auth/register/route.ts#L1-L56)
- [app/api/auth/verify-email/route.ts:1-38](file://app/api/auth/verify-email/route.ts#L1-L38)
- [app/api/auth/forgot-password/route.ts:1-34](file://app/api/auth/forgot-password/route.ts#L1-L34)
- [app/api/auth/reset-password/route.ts:1-31](file://app/api/auth/reset-password/route.ts#L1-L31)
- [app/api/auth/set-password/route.ts:1-27](file://app/api/auth/set-password/route.ts#L1-L27)

## 结论
- 当前认证体系以 Cookie+JWT 为核心，中间件负责页面级鉴权与重定向
- **新增**：已登录用户访问认证页面时的智能重定向机制，提升用户体验
- **新增**：详细的调试日志记录功能，支持路径名、认证状态、令牌存在性和令牌预览信息的追踪
- 登录、注册、邮箱验证、密码重置等流程完善，具备基本的安全措施
- 建议在生产环境强化 token 校验、启用 HTTPS、细化角色权限，并补充安全头与 CORS 策略
- 建议在生产环境调整调试日志级别，避免性能影响

## 附录

### 安全头设置与 CORS 配置建议
- 安全头
  - Content-Security-Policy：限制脚本与资源加载来源
  - X-Content-Type-Options: nosniff
  - X-Frame-Options: DENY 或 SAMEORIGIN
  - Referrer-Policy: strict-origin-when-cross-origin
  - Permissions-Policy：禁用不必要功能
- CORS
  - 明确允许的源、方法与头部
  - 针对敏感接口启用 credentials 与精确域名白名单
- 部署与环境
  - 生产环境务必启用 HTTPS，并将 Cookie secure=true
  - 使用环境变量管理 JWT_SECRET，避免硬编码
  - **新增**：生产环境建议禁用详细调试日志或设置合适的日志级别