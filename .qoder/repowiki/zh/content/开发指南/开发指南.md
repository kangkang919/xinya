# 开发指南

<cite>
**本文引用的文件**   
- [README.md](file://README.md)
- [package.json](file://package.json)
- [tsconfig.json](file://tsconfig.json)
- [eslint.config.mjs](file://eslint.config.mjs)
- [.gitignore](file://.gitignore)
- [AGENTS.md](file://AGENTS.md)
- [CLAUDE.md](file://CLAUDE.md)
- [next.config.ts](file://next.config.ts)
- [middleware.ts](file://middleware.ts)
- [lib/auth.ts](file://lib/auth.ts)
- [lib/prisma.ts](file://lib/prisma.ts)
- [types/index.ts](file://types/index.ts)
- [prisma/schema.prisma](file://prisma/schema.prisma)
- [scripts/backfill-questions.ts](file://scripts/backfill-questions.ts)
- [scripts/create-guest-accounts.js](file://scripts/create-guest-accounts.js)
- [scripts/create-migration.js](file://scripts/create-migration.js)
- [scripts/regenerate-all-summaries.ts](file://scripts/regenerate-all-summaries.ts)
- [ecosystem.config.js](file://ecosystem.config.js)
- [deploy.sh](file://deploy.sh)
- [doc/新芽dev-framework.md](file://doc/新芽dev-framework.md)
</cite>

## 更新摘要
**所做更改**   
- 新增了"程序转移与项目交接规范"章节，包含离职前检查清单、敏感信息处理指南和账户密钥管理流程
- 更新了贡献代码的参与指南，增加了团队过渡期间的安全交接要求
- 完善了开发约束规范，强调项目交接时的文档完整性和安全性检查
- 增强了变更记录部分，添加了程序转移相关的规范要求

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本指南面向心芽项目的开发者，覆盖从环境搭建、IDE与代码规范配置，到 TypeScript 类型实践、ESLint 规则、Git 工作流、新功能开发流程、测试编写、性能分析与调试技巧、常见问题排查以及贡献规范等全链路内容。**特别强调开发约束与问题诊断规范，以及程序转移与项目交接的安全要求**。目标是帮助新成员快速上手并高质量地参与迭代，同时确保团队成员变动时的项目安全交接。

## 项目结构
本项目基于 Next.js App Router，采用前后端一体化工程：
- 前端页面与布局位于 app 目录，按路由分组组织
- API 路由位于 app/api，按功能域划分
- 共享逻辑与工具在 lib 目录
- **开发工具脚本位于 scripts 目录，现已被 Git 版本控制跟踪**
- 数据库模型与迁移在 prisma 目录
- 全局类型定义在 types 目录
- 构建与运行脚本在 package.json
- **部署配置文件位于根目录，包括 PM2 配置和部署脚本**

```mermaid
graph TB
subgraph "应用层"
A["app/ 页面与布局"]
B["app/api/ API 路由"]
end
subgraph "服务与工具"
C["lib/ 业务与工具"]
D["types/ 类型定义"]
E["scripts/ 开发工具脚本"]
end
subgraph "数据层"
F["prisma/ 模型与迁移"]
end
subgraph "配置"
G["next.config.ts"]
H["tsconfig.json"]
I["eslint.config.mjs"]
J[".gitignore"]
K["ecosystem.config.js"]
L["deploy.sh"]
end
A --> B
B --> C
C --> F
A --> D
B --> D
E --> C
E --> F
G --> A
G --> B
H --> A
H --> B
I --> A
I --> B
J --> A
J --> B
J --> E
K --> G
L --> K
```

**图表来源**
- [next.config.ts:1-8](file://next.config.ts#L1-L8)
- [tsconfig.json:1-35](file://tsconfig.json#L1-L35)
- [eslint.config.mjs:1-19](file://eslint.config.mjs#L1-L19)
- [.gitignore:1-20](file://.gitignore#L1-L20)
- [ecosystem.config.js:1-15](file://ecosystem.config.js#L1-L15)
- [deploy.sh:1-36](file://deploy.sh#L1-L36)

**章节来源**
- [README.md:1-37](file://README.md#L1-L37)
- [package.json:1-40](file://package.json#L1-L40)

## 核心组件
- 认证与安全
  - JWT 签发与校验、Cookie 设置、密码哈希与验证集中在认证模块中，供 API 路由复用。
- 数据库访问
  - Prisma Client 单例化，开发环境开启查询日志，生产仅记录错误。
- 中间件鉴权
  - 对非静态资源与非 API 路径进行登录态检查，未登录重定向至登录页。
- 类型系统
  - 统一的前端类型定义，包括心情、主题、卡片、详情、标签、今日速览与通用响应结构。
- **开发工具脚本**
  - 提供批量数据处理、用户创建、数据库迁移等实用工具脚本，现已被版本控制跟踪便于协作。
- **部署配置**
  - PM2 进程管理配置，支持 Node.js 原生环境变量加载和生产环境优化。

**章节来源**
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)
- [lib/prisma.ts:1-14](file://lib/prisma.ts#L1-L14)
- [middleware.ts:1-29](file://middleware.ts#L1-L29)
- [types/index.ts:1-48](file://types/index.ts#L1-L48)
- [ecosystem.config.js:1-15](file://ecosystem.config.js#L1-L15)

## 架构总览
Next.js 作为运行时承载页面渲染与 API 处理；中间件负责请求级鉴权；API 路由调用认证与数据库服务；Prisma 连接 PostgreSQL；全局类型保障前后端契约一致性；**开发工具脚本提供数据管理和维护支持；PM2 配置确保生产环境稳定运行**。

```mermaid
graph TB
U["浏览器"]
MW["中间件<br/>middleware.ts"]
API["API 路由<br/>app/api/*"]
AUTH["认证模块<br/>lib/auth.ts"]
DB["Prisma 客户端<br/>lib/prisma.ts"]
PG["PostgreSQL<br/>DATABASE_URL"]
SCRIPTS["开发工具脚本<br/>scripts/*"]
PM2["PM2 进程管理<br/>ecosystem.config.js"]
U --> MW
MW --> |放行或重定向| API
API --> AUTH
API --> DB
DB --> PG
SCRIPTS --> DB
SCRIPTS --> AUTH
PM2 --> API
```

**图表来源**
- [middleware.ts:1-29](file://middleware.ts#L1-L29)
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)
- [lib/prisma.ts:1-14](file://lib/prisma.ts#L1-L14)
- [prisma/schema.prisma:1-209](file://prisma/schema.prisma#L1-L209)
- [ecosystem.config.js:1-15](file://ecosystem.config.js#L1-L15)

## 详细组件分析

### 认证与安全（JWT + Cookie）
- 职责
  - 密码哈希与校验
  - JWT 签发与校验
  - 从请求上下文获取当前用户 ID
  - Cookie 配置（名称、过期时间、安全属性）
- 关键要点
  - 使用 bcrypt 进行密码哈希，强度固定为 12
  - JWT 有效期 30 天，密钥来自环境变量
  - Cookie httpOnly 提升安全性，sameSite 设为 lax，path 为根路径
  - getCurrentUserId 封装了从 cookies 读取 token 并解析的完整流程

```mermaid
sequenceDiagram
participant C as "客户端"
participant M as "中间件"
participant A as "API 路由"
participant S as "认证模块"
participant K as "密钥/环境变量"
C->>M : 发起受保护页面请求
M->>M : 判断是否为鉴权页面或静态资源
alt 需要鉴权且无 Token
M-->>C : 重定向到 /login
else 通过
M-->>A : 放行
A->>S : 解析 Cookie 中的 Token
S->>K : 读取 JWT_SECRET
S-->>A : 返回 userId 或 null
A-->>C : 返回受保护数据
end
```

**图表来源**
- [middleware.ts:1-29](file://middleware.ts#L1-L29)
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)

**章节来源**
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)
- [middleware.ts:1-29](file://middleware.ts#L1-L29)

### 数据库访问（Prisma 单例）
- 职责
  - 初始化 PrismaClient 并注入日志级别
  - 在非生产环境下缓存实例，避免热重载重复创建
- 关键要点
  - 开发环境输出 query/error/warn，便于定位慢查询与异常
  - 生产环境仅输出 error，减少日志开销
  - 通过环境变量 DATABASE_URL 连接 PostgreSQL

```mermaid
flowchart TD
Start(["进程启动"]) --> CheckEnv["检测 NODE_ENV"]
CheckEnv --> Dev{"是否开发环境?"}
Dev --> |是| InitDev["初始化 PrismaClient(开启 query/error/warn)"]
Dev --> |否| InitProd["初始化 PrismaClient(仅 error)"]
InitDev --> Cache["缓存到 globalThis"]
InitProd --> Cache
Cache --> Use["后续模块复用同一实例"]
```

**图表来源**
- [lib/prisma.ts:1-14](file://lib/prisma.ts#L1-L14)

**章节来源**
- [lib/prisma.ts:1-14](file://lib/prisma.ts#L1-L14)

### 中间件鉴权流程
- 职责
  - 过滤静态资源与 API 路径
  - 白名单鉴权页面直接放行
  - 其他页面若无 xinya_token 则重定向登录
- 关键要点
  - matcher 排除静态资源与 favicon 等
  - 鉴权页面集合包含登录、注册、邮箱验证、忘记密码、重置密码、引导页与展示页
  - **注意：禁止在 app/page.tsx 中使用无条件 redirect()，会覆盖路由组**

```mermaid
flowchart TD
Entry(["进入中间件"]) --> Path["解析 pathname"]
Path --> IsAPI{"是否 /api/* 或 /_next/* ?"}
IsAPI --> |是| Next1["放行"]
IsAPI --> |否| HasToken{"是否存在 xinya_token ?"}
HasToken --> |是| Next2["放行"]
HasToken --> |否| Redirect["重定向到 /login"]
```

**图表来源**
- [middleware.ts:1-29](file://middleware.ts#L1-L29)

**章节来源**
- [middleware.ts:1-29](file://middleware.ts#L1-L29)

### 类型系统与契约
- 职责
  - 统一定义心情、主题、卡片、详情、标签、今日速览与通用响应结构
- 关键要点
  - 列表项与详情页分离，前者精简字段，后者包含富文本内容
  - ApiResponse 提供统一的 ok/data/error 结构，便于前端消费

```mermaid
classDiagram
class EntryCard {
+string id
+string title
+string contentPreview
+TagItem[] tags
+MoodType mood
+string recordTime
+boolean isTop
+boolean isFavorite
+boolean isDraft
}
class EntryDetail {
+string content
}
class TagItem {
+string id
+string name
+boolean isDefault
+number entryCount
}
class TodaySummary {
+number todayCount
+number weekCount
+number streak
+number maxStreak
+{title : string} lastEntry
}
class ApiResponse~T~ {
+boolean ok
+T data
+string error
}
EntryDetail --|> EntryCard : "扩展"
```

**图表来源**
- [types/index.ts:1-48](file://types/index.ts#L1-L48)

**章节来源**
- [types/index.ts:1-48](file://types/index.ts#L1-L48)

### 数据库模型概览
- 核心实体
  - User、Entry、Tag、Share、AiInsight、InsightReport、GrowthLog、EmailToken、MagicLink、QuizQuestion、QuizRecord、UserSetting、ReviewCallLog
- 关系与索引
  - 多对一/一对多关系清晰，常用查询字段建立索引以提升性能
  - 唯一约束保证业务键不重复（如 email、token、userId+name 等）

```mermaid
erDiagram
USER ||--o{ ENTRY : "拥有"
USER ||--o{ TAG : "拥有"
USER ||--o{ SHARE : "拥有"
USER ||--o{ AIINSIGHT : "生成"
USER ||--o{ INSIGHTREPORT : "生成"
USER ||--o{ GROWTHLOG : "记录"
USER ||--o{ EMAILTOKEN : "持有"
USER ||--o| USERSETTING : "配置"
USER ||--o{ QUIZRECORD : "作答"
USER ||--o{ REVIEWCALLLOG : "调用"
ENTRY ||--o{ TAG : "标记"
ENTRY ||--o{ QUIZQUESTION : "包含"
QUIZQUESTION ||--o{ QUIZRECORD : "被作答"
```

**图表来源**
- [prisma/schema.prisma:1-209](file://prisma/schema.prisma#L1-L209)

**章节来源**
- [prisma/schema.prisma:1-209](file://prisma/schema.prisma#L1-L209)

### 开发工具脚本（新增）
**更新** 由于 .gitignore 配置更新，scripts 目录现已被 Git 版本控制跟踪，所有开发工具脚本都纳入团队协作范围。

#### 脚本概览
- **backfill-questions.ts**: 补生成心得的题目和要点总结，支持 AI 生成和模板降级
- **create-guest-accounts.js**: 批量创建测试用访客账户
- **create-migration.js**: 生成数据库迁移文件
- **regenerate-all-summaries.ts**: 批量重新生成所有心得的 AI 总结和题目

#### 脚本特性
- **版本控制集成**: 所有脚本现可被 Git 跟踪，便于团队协作和代码审查
- **错误处理**: 完善的异常处理和日志输出
- **数据一致性**: 确保数据操作的原子性和完整性
- **API 集成**: 支持 DeepSeek AI 服务进行智能内容生成

#### 使用示例
```bash
# 补生成缺失的题目和要点
npx tsx scripts/backfill-questions.ts

# 批量创建测试账户
node scripts/create-guest-accounts.js

# 生成数据库迁移
node scripts/create-migration.js

# 重新生成所有心得的 AI 总结
npx tsx scripts/regenerate-all-summaries.ts
```

**章节来源**
- [scripts/backfill-questions.ts:1-261](file://scripts/backfill-questions.ts#L1-L261)
- [scripts/create-guest-accounts.js:1-59](file://scripts/create-guest-accounts.js#L1-L59)
- [scripts/create-migration.js:1-122](file://scripts/create-migration.js#L1-L122)
- [scripts/regenerate-all-summaries.ts:1-179](file://scripts/regenerate-all-summaries.ts#L1-L179)

### 部署配置（更新）
**更新** PM2 部署配置已优化，现在使用 Node.js 原生 --env-file 参数加载环境变量，提高了安全性和稳定性。

#### PM2 配置要点
- **环境变量加载**: 使用 `--env-file=/www/wwwroot/xinya/.env.production` 直接加载生产环境配置
- **进程管理**: 单实例运行，自动重启，内存限制 512MB
- **工作目录**: 指定 `/www/wwwroot/xinya` 作为工作目录
- **生产环境**: 设置 `NODE_ENV=production` 启用生产模式优化

#### 部署脚本流程
1. 安装依赖 (`npm install`)
2. 生成 Prisma Client (`npx prisma generate`)
3. 执行数据库迁移 (`npx prisma migrate deploy`)
4. 构建项目 (`npm run build`)
5. 启动 PM2 进程管理

**章节来源**
- [ecosystem.config.js:1-15](file://ecosystem.config.js#L1-L15)
- [deploy.sh:1-36](file://deploy.sh#L1-L36)

## 依赖分析
- 运行时依赖
  - next、react/react-dom 用于页面与交互
  - @prisma/client 与 prisma 用于数据库访问与迁移
  - bcryptjs、jsonwebtoken 用于密码与令牌
  - nodemailer 用于邮件发送
  - lucide-react 图标库、react-hot-toast 提示
  - dotenv 用于环境变量加载
- 开发依赖
  - eslint、eslint-config-next 提供代码质量规则
  - tailwindcss 与 @tailwindcss/postcss 样式方案
  - typescript 与各类 @types 提供类型支持

```mermaid
graph LR
P["package.json 脚本与依赖"] --> N["Next.js 应用"]
P --> ESL["ESLint 规则"]
P --> TS["TypeScript 编译"]
P --> PRISMA["Prisma 客户端/CLI"]
N --> AUTH["认证模块"]
N --> DB["Prisma 客户端"]
AUTH --> JWT["jsonwebtoken"]
AUTH --> BC["bcryptjs"]
DB --> PG["PostgreSQL"]
SCRIPTS["开发脚本"] --> DB
SCRIPTS --> AUTH
ENV["dotenv 环境变量"] --> N
```

**图表来源**
- [package.json:1-40](file://package.json#L1-L40)

**章节来源**
- [package.json:1-40](file://package.json#L1-L40)

## 性能考虑
- 数据库
  - 利用现有索引（如按 userId 与时间排序）优化高频查询
  - 按需 select 字段，避免拉取大字段（如富文本）
- 应用层
  - 中间件仅做必要判断，避免复杂计算
  - 将耗时操作（如邮件发送、AI 分析）异步化或队列化
- 构建与运行
  - 生产环境关闭多余日志，减少 I/O 开销
  - 合理使用增量编译与缓存策略
  - **PM2 配置内存限制防止内存泄漏导致崩溃**
- **脚本优化**
  - 批量处理时添加适当的延迟避免 API 限流
  - 使用事务确保数据一致性
  - 实现断点续传和错误恢复机制

[本节为通用建议，无需源码引用]

## 故障排查指南
- 无法启动或端口占用
  - 确认 3000 端口未被占用，必要时更换端口
- 数据库连接失败
  - 检查 DATABASE_URL 是否正确，PostgreSQL 服务是否运行
  - 查看 Prisma 日志（开发模式已开启 query/error/warn）
- 鉴权失败或频繁跳转登录
  - 检查 Cookie 是否携带 xinya_token，域名与 SameSite 设置是否符合预期
  - 确认 JWT_SECRET 一致且未过期
  - **注意：检查是否有在 app/page.tsx 中使用无条件 redirect() 导致的路由冲突**
- 邮件发送失败
  - 核对 SMTP 服务器、端口、授权码与账号配置
- 构建失败
  - 清理 .next 与 node_modules 后重装依赖，重新生成 Prisma Client
- **脚本执行问题**
  - 确认环境变量配置正确（特别是 API 密钥）
  - 检查网络连接和 API 服务可用性
  - 查看详细的错误日志输出
- **PM2 部署问题**
  - 检查 `.env.production` 文件路径是否正确
  - 确认 Node.js 版本兼容性
  - 查看 PM2 日志：`pm2 logs xinya`

### 标准化问题排查流程
**更新** 根据新的开发约束规范，遇到问题时应遵循以下标准流程：

#### 问题诊断第一原则
发现 bug 时，第一步必须判断问题来源：

| 问题类型 | 判断标准 | 排查方式 |
|---------|---------|---------|
| **历史遗留** | 功能一直有问题，今天才发现 | 按问题现象直接排查修复 |
| **迭代引入** | 之前正常，某次改动后出问题 | **必须回溯 git 历史 diff 对比** |

#### 迭代问题的标准排查流程
```
1. git log --oneline --since="YYYY-MM-DD"  → 找到时间范围内的所有commit
2. 确定最后正常工作的commit（用户确认或测试验证）
3. git diff <正常版本>..<当前版本> -- <关键文件>  → 精确对比差异
4. 重点关注：新增文件、路由文件、配置文件、环境变量变更
5. 不要盲目添加修复代码，先理解"什么改动导致了问题"
```

#### Next.js 路由优先级警示
- `app/page.tsx` 优先级高于 `app/(group)/page.tsx`
- **禁止**在 `app/page.tsx` 中使用无条件 `redirect()`，会覆盖路由组
- 未登录跳转逻辑应放在 middleware 中处理，不在页面组件中

#### 禁止盲目叠加修复
- 不确定根因时，不要同时改多个文件
- 每次修改只改一个地方，验证后再改下一个
- 修改前先问自己："这个改动是否可能引入新问题？"

**章节来源**
- [lib/prisma.ts:1-14](file://lib/prisma.ts#L1-L14)
- [lib/auth.ts:1-56](file://lib/auth.ts#L1-L56)
- [middleware.ts:1-29](file://middleware.ts#L1-L29)
- [doc/新芽dev-framework.md:840-883](file://doc/新芽dev-framework.md#L840-L883)

## 结论
本指南梳理了心芽项目的开发环境与工程规范，明确了认证、数据库、中间件与类型系统的职责边界与实践要点。**随着新增的开发约束与问题诊断规范的加入，以及程序转移与项目交接安全要求的完善，团队协作效率得到进一步提升**。遵循本文档的流程与规范，特别是严格的问题诊断流程和路由优先级警示，以及项目交接时的安全检查，可显著提升协作效率与代码质量。

[本节为总结性内容，无需源码引用]

## 附录

### 开发环境搭建
- 前置要求
  - Node.js 与包管理器（npm/yarn/pnpm/bun 任选其一）
  - PostgreSQL 数据库服务
- 安装与启动
  - 克隆仓库并安装依赖
  - 生成 Prisma Client
  - 启动开发服务器并在浏览器访问本地地址
- 环境变量
  - 复制模板并填写真实值（数据库连接、JWT 密钥、SMTP 等）
  - 注意 .env 系列文件已被 .gitignore 排除，切勿提交

**章节来源**
- [README.md:1-37](file://README.md#L1-L37)
- [package.json:1-40](file://package.json#L1-L40)
- [.gitignore:1-20](file://.gitignore#L1-L20)

### IDE 与编辑器配置
- 推荐插件
  - ESLint、Prettier（可选）、Tailwind CSS IntelliSense、Prisma VS Code Extension
- 保存时自动格式化与修复
  - 启用保存时运行 ESLint 修复
  - 若使用 Prettier，确保与 ESLint 规则兼容
- 调试
  - 使用浏览器开发者工具调试前端
  - 使用 Next.js DevTools 与 Network 面板观察请求
  - 后端可在 API 路由内添加断点调试
  - **脚本调试可使用 Node.js 调试器或控制台日志**

[本节为通用建议，无需源码引用]

### TypeScript 配置与类型最佳实践
- tsconfig 要点
  - 严格模式开启，目标 ES2017，模块解析为 bundler
  - 路径别名 @/* 指向项目根目录
  - include 覆盖 .next 生成的类型，exclude node_modules
- 类型实践
  - 优先使用 interface 描述对象结构，type 用于联合与映射
  - 对外暴露的类型集中放在 types 目录，保持契约稳定
  - 对第三方库使用 @types 声明，避免 any

**章节来源**
- [tsconfig.json:1-35](file://tsconfig.json#L1-L35)
- [types/index.ts:1-48](file://types/index.ts#L1-L48)

### ESLint 规则与自定义
- 规则来源
  - 继承 next/core-web-vitals 与 next/typescript 两套规则集
- 忽略策略
  - 通过 globalIgnores 覆盖默认忽略，保留 .next、out、build 与 next-env.d.ts 不被检查
- 自定义建议
  - 新增规则时尽量以插件形式引入，保持可读性与可维护性
  - 结合编辑器实现保存时自动修复

**章节来源**
- [eslint.config.mjs:1-19](file://eslint.config.mjs#L1-L19)

### Git 工作流与提交规范
**更新** 由于 .gitignore 配置更新，scripts 目录现已被版本控制跟踪。

- 分支策略
  - main 为主分支，feature/* 用于功能开发，hotfix/* 用于紧急修复
- 提交信息
  - 使用约定式提交格式：type(scope): subject
  - type 示例：feat、fix、docs、style、refactor、test、chore
- **脚本管理**
  - 所有开发工具脚本现需遵循相同的代码规范和提交标准
  - 脚本变更应包含详细的变更说明和使用示例
  - 重要脚本应添加相应的单元测试
- 双平台同步
  - 远程 origin 指向 GitHub，gitee 指向 Gitee
  - 推送时需分别推送到两个远程仓库

**章节来源**
- [.gitignore:1-20](file://.gitignore#L1-L20)

### 新功能开发流程与代码审查标准
**更新** 新增脚本开发和测试要求，强化开发约束规范。

- 开发流程
  - **开发前必读**：每次新建开发需求时，必须先读取 dev-framework.md 文档，特别是附录 A-0 开发约束规范
  - 从 main 拉取最新代码，创建 feature 分支
  - 完成功能开发与自测，更新相关文档与类型
  - **如需开发工具脚本，遵循脚本开发规范并进行充分测试**
  - 提交变更并推送远端，发起 Pull Request
- 审查标准
  - 代码风格符合 ESLint 规则
  - 新增/修改类型需同步更新 types 与接口契约
  - 涉及鉴权与权限控制的路径需补充中间件或守卫
  - 数据库变更需附带迁移与回滚说明
  - 性能敏感路径需提供压测或监控指标
  - **脚本需包含完整的错误处理和日志输出**
  - **脚本变更需说明影响范围和回滚方案**
  - **严禁在 app/page.tsx 中使用无条件 redirect()**

[本节为通用建议，无需源码引用]

### 单元测试与集成测试指南
**更新** 增加脚本测试要求。

- 单元与组件测试
  - 针对纯函数与工具方法编写用例，覆盖边界条件
  - 组件测试关注渲染结果与交互行为
- API 集成测试
  - 模拟数据库与外部服务（如邮件），验证端到端流程
  - 使用事务或测试数据库隔离数据
- **脚本测试**
  - 为脚本的关键逻辑编写单元测试
  - 模拟数据库操作和外部 API 调用
  - 测试错误处理和异常情况
- 持续集成
  - 在 CI 中执行 lint、类型检查与测试套件

[本节为通用建议，无需源码引用]

### 性能分析与调试技巧
- 前端
  - React Profiler 分析渲染瓶颈
  - Lighthouse 评估性能与可访问性
- 后端
  - 开启 Prisma 查询日志定位慢查询
  - 使用浏览器 Network 面板分析请求耗时与体积
- 线上
  - 结合 PM2 日志与错误上报系统进行问题追踪
  - **使用 pm2 logs xinya 查看生产环境日志**
- **脚本调试**
  - 使用 console.log 和结构化日志输出
  - 实现进度跟踪和错误统计
  - 支持断点续传和重试机制

**章节来源**
- [lib/prisma.ts:1-14](file://lib/prisma.ts#L1-L14)
- [ecosystem.config.js:1-15](file://ecosystem.config.js#L1-L15)

### 程序转移与项目交接规范（新增）
**新增** 为确保团队成员变动时的项目安全和知识传承，制定以下程序转移与项目交接规范。

#### 离职前检查清单
- **代码与文档**
  - 确保所有功能代码已提交并合并到主分支
  - 更新相关技术文档和设计文档
  - 清理个人本地的临时文件和调试配置
- **权限与账户**
  - 移交所有项目相关账户的管理权限
  - 移除个人在CI/CD流水线中的访问权限
  - 撤销个人对生产环境的直接访问权限
- **敏感信息处理**
  - 清除本地存储的所有敏感配置文件（.env、私钥等）
  - 删除个人电脑上的SSH密钥和证书
  - 清理浏览器中保存的项目相关凭据
- **知识传承**
  - 完成关键技术点的文档说明
  - 与接手人员进行面对面的项目讲解
  - 提供常见问题的解决方案文档

#### 敏感信息处理指南
- **配置文件管理**
  - 所有敏感配置必须使用环境变量管理
  - 禁止在代码中硬编码任何敏感信息
  - 定期轮换API密钥和数据库密码
- **密钥轮换流程**
  - 离职前必须完成所有相关密钥的轮换
  - 更新所有使用该密钥的服务和应用程序
  - 验证新密钥在所有环境中的正常工作
- **数据清理**
  - 删除本地数据库的备份文件
  - 清理测试数据和临时文件
  - 确保没有敏感信息残留在日志文件中

#### 账户密钥摘要
- **必需移交的账户**
  - 代码托管平台（GitHub/GitLab）管理员权限
  - 云服务提供商账户（阿里云/AWS等）
  - 域名和SSL证书管理账户
  - 第三方服务API密钥（邮件服务、AI服务等）
- **密钥轮换检查表**
  - [ ] 数据库连接字符串已更新
  - [ ] JWT密钥已重新生成
  - [ ] 邮件服务API密钥已轮换
  - [ ] AI服务API密钥已更新
  - [ ] 云存储服务密钥已轮换
  - [ ] 监控和分析服务密钥已更新
- **交接验证流程**
  - 新成员能够独立部署和运行项目
  - 所有关键服务都能正常访问
  - 紧急情况下能够快速恢复服务

**章节来源**
- [doc/新芽dev-framework.md:840-883](file://doc/新芽dev-framework.md#L840-L883)

### 贡献代码的参与指南与规范要求
**更新** 增加脚本贡献要求和开发约束规范，以及项目交接安全要求。

- 行为准则
  - 尊重他人，友善沟通，遵守社区规范
- 贡献方式
  - 报告 Bug、提出改进建议、完善文档
  - 提交代码前确保通过 lint 与类型检查
  - **脚本贡献需包含完整的使用说明和测试用例**
- 合规要求
  - 不提交敏感信息（密钥、凭据等）
  - 遵循提交信息与分支命名规范
  - **脚本变更需考虑向后兼容性和数据安全**
  - **严格遵守开发约束与问题诊断规范**
  - **参与项目交接时必须完成敏感信息清理和权限移交**

[本节为通用建议，无需源码引用]

### 参考与提示
- Next.js 版本差异提示
  - 请阅读 AGENTS.md 中的注意事项，遵循官方文档的最新约定

**章节来源**
- [AGENTS.md:1-6](file://AGENTS.md#L1-L6)
- [CLAUDE.md:1-2](file://CLAUDE.md#L1-L2)

### 开发约束与问题诊断规范（必读）
**新增** 以下规则从实际踩坑中总结，每次开发前必须回顾。

#### 规则 1：开发前必须核对本文档
每次新建开发需求时，**必须先读取本文档**，逐条核对：
- 新需求是否与已确认功能冲突
- 改动是否会影响已有功能（特别是路由、中间件、配置文件、环境变量）
- 有冲突的必须经过用户确认是否继续
- 开发完成后**必须同步更新**本文档的变更记录（附录 A）

#### 规则 2：问题诊断第一原则
发现 bug 时，第一步必须判断问题来源：

| 问题类型 | 判断标准 | 排查方式 |
|---------|---------|---------|
| **历史遗留** | 功能一直有问题，今天才发现 | 按问题现象直接排查修复 |
| **迭代引入** | 之前正常，某次改动后出问题 | **必须回溯 git 历史 diff 对比** |

#### 规则 3：迭代问题的标准排查流程
```
1. git log --oneline --since="YYYY-MM-DD"  → 找到时间范围内的所有commit
2. 确定最后正常工作的commit（用户确认或测试验证）
3. git diff <正常版本>..<当前版本> -- <关键文件>  → 精确对比差异
4. 重点关注：新增文件、路由文件、配置文件、环境变量变更
5. 不要盲目添加修复代码，先理解"什么改动导致了问题"
```

#### 规则 4：Next.js 路由优先级警示
- `app/page.tsx` 优先级高于 `app/(group)/page.tsx`
- **禁止**在 `app/page.tsx` 中使用无条件 `redirect()`，会覆盖路由组
- 未登录跳转逻辑应放在 middleware 中处理，不在页面组件中

#### 规则 5：禁止盲目叠加修复
- 不确定根因时，不要同时改多个文件
- 每次修改只改一个地方，验证后再改下一个
- 修改前先问自己："这个改动是否可能引入新问题？"

**章节来源**
- [doc/新芽dev-framework.md:840-883](file://doc/新芽dev-framework.md#L840-L883)

### 变更记录
**更新** 添加了最新的拾遗系统修复、PM2部署配置更新、认证流程修复以及程序转移规范等变更记录。

| 日期 | 变更内容 | 状态 |
| :--- | :--- | :--- |
| 2026-06-21 | 置顶规则从"最多1篇"改为"允许多篇同时置顶" | 已验收 |
| 2026-06-21 | 移除"存草稿"功能与按钮 | 已验收 |
| 2026-06-21 | 新增根系页 Markdown/HTML 导出 | 已验收 |
| 2026-06-21 | 标签强制存在规则 | 已验收 |
| 2026-06-21 | 时间统计统一为北京时间 | 已验收 |
| 2026-06-21 | 四页标题/slogan 对齐 | 已验收 |
| 2026-06-21 | 手机端编辑工具栏固定 | 已验收 |
| 2026-06-21 | PWA 配置：名称"心芽"、图标、manifest | 已验收 |
| 2026-06-21 | 代码管理升级为 GitHub + 服务器部署 | 已确认 |
| 2026-06-21 | 创建新芽 dev-framework v2.1 | 已确认 |
| 2026-06-30 | 域名 shuxiangnote.top 上线，HTTPS + 反向代理配置完成 | 已验收 |
| 2026-06-30 | PWA 图标去白边，绿色背景铺满全图 | 已验收 |
| 2026-07-01 | 年轮页改为传统日历网格布局（周一到周日横排，显示日期号） | 已验收 |
| 2026-07-01 | 年轮页去掉空状态文案，始终显示完整日历网格 | 已验收 |
| 2026-07-01 | 统计卡片固定行高（minHeight: 80px） | 已验收 |
| 2026-07-01 | AI 服务从硅基流动切换为 DeepSeek | 已确认 |
| 2026-07-01 | 新增 F9 拾遗（每日概念卡片）功能设计 | 已确认 |
| 2026-07-01 | F7 登录改为 Magic Link 方案（邮箱验证码→邮件链接一键登录） | 已确认 |
| 2026-07-01 | 新增 F10 邀请码方案（预生成 15 个账号） | 已确认 |
| 2026-07-01 | 新增第 5 套主题「暗夜」深色主题（暗绿强调色系） | 已确认 |
| 2026-07-01 | F4.3 枝叶页标签筛选功能修复完成 | 已验收 |
| 2026-07-01 | F5.7 年轮页累计篇数实现 | 已验收 |
| 2026-07-01 | F5.8 拾遗设置入口移至根系页 | 已验收 |
| 2026-07-01 | F6.5 暗夜主题完整实现 | 已验收 |
| 2026-07-01 | F7.1-F7.4 Magic Link 登录/注册完整实现 | 已验收 |
| 2026-07-01 | F9 拾遗功能后端+前端实现（调试中） | 进行中 |
| 2026-06-21 | 暗色主题持久化修复：登录流程无条件写入 theme 到 localStorage + 双切模式（spring→用户主题）强制生效 | 已验收 |
| 2026-06-21 | 枝叶页暗色主题适配：搜索框/心得列表使用 useTheme 变量（inputBg/cardBg） | 已验收 |
| 2026-06-21 | 枝叶页标签级导航返回：URL tagId 参数传递，阅读页返回时自动恢复标签选中状态 | 已验收 |
| 2026-06-21 | 根系页导出功能修订：仅 Markdown 格式，内容含标题/正文/标签/创建时间，位置移至版本更新上方 | 已验收 |
| 2026-06-21 | 根系页暗色主题全适配：所有卡片/文字使用 useTheme 变量（cardBg/cardBorder/titleColor/dimColor） | 已验收 |
| 2026-06-21 | 根系页主题选择补回「墨色幽微」按钮 | 已验收 |
| 2026-07-09 | F9.15 拾遗弹窗 AI 总结文案靠左+两边对齐 | 已验收 |
| 2026-07-09 | F9.16 答题回看面板：三态展示（✓你选了/△漏选/误选）+ 解析 | 已验收 |
| 2026-07-09 | F9.17 学习画像：根系页展示学习天数/答题总数（累计次数）/准确率/近5日/薄弱领域/掌握良好 | 已验收 |
| 2026-07-09 | F9.18 心得详情页返回逻辑：URL from 参数区分来源（萌芽/AI弹窗→萌芽首页，枝叶→枝叶页保留标签） | 已验收 |
| 2026-07-09 | F6.5 主题精简为 2 套（春日萌芽/墨色幽微），默认春日萌芽，所有默认值统一 | 已验收 |
| 2026-07-09 | quiz_records 表新增 userAnswer（Json?）和 answerCount（Int）字段 | 已验收 |
| 2026-07-14 | F9 拾遗4项修复：准确率计算口径统一、编辑心得触发AI重新生成、AI总结150字、答题卡随机选取 | 已验收 |
| 2026-07-15 | 批量重生成所有心得AI总结(150字)和题目脚本 | 已验收 |
| 2026-07-15 | 枝叶页编辑后返回原滚动位置，标签变更后点击标签才刷新列表 | 已验收 |
| 2026-07-15 | 编辑页添加Suspense包裹useSearchParams（修复prerender错误） | 已验收 |
| 2026-07-15 | PM2部署改用Node.js原生--env-file加载环境变量 | 已验收 |
| 2026-07-15 | 删除app/page.tsx无条件redirect，修复登录后被踢回登录页问题 | 已验收 |
| 2026-07-16 | 新增程序转移与项目交接规范，包含离职前检查清单、敏感信息处理指南和账户密钥摘要 | 已完成 |

**章节来源**
- [doc/新芽dev-framework.md:886-934](file://doc/新芽dev-framework.md#L886-L934)