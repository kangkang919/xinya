...
- [doc/心芽项目资料索引.md](file://doc/心芽项目资料索引.md)
</cite>

## 更新摘要
**所做更改**
- 增强了项目结构概览，基于最新的项目资料索引文档添加了完整的目录结构信息
- 更新了技术栈说明，包含更详细的技术选型和版本信息
- 完善了功能模块描述，涵盖认证、条目管理、复习系统、AI集成等核心功能
- 改进了快速参考部分，提供更清晰的开发工作流指南
- 增强了架构设计说明，突出API优先策略和组件化设计理念
...
## 项目结构
整体采用 Next.js App Router 的分层组织方式，基于最新的项目资料索引文档，完整目录结构如下：
...
**更新** 基于项目资料索引文档，新增了详细的目录结构说明和功能模块划分

```mermaid
graph TB
subgraph "前端层"
UI["页面与组件<br/>app/*, components/*"]
Layout["布局系统<br/>layout.tsx"]
end
subgraph "API层"
AuthAPI["认证API<br/>api/auth/*"]
EntryAPI["条目API<br/>api/entries/*"]
ReviewAPI["复习API<br/>api/review/*"]
TagAPI["标签API<br/>api/tags/*"]
end
subgraph "业务逻辑层"
AuthService["认证服务<br/>lib/auth.ts"]
DeepSeekService["AI服务<br/>lib/deepseek.ts"]
Utils["工具函数<br/>lib/utils.ts"]
end
subgraph "数据层"
PrismaClient["Prisma客户端<br/>lib/prisma.ts"]
...
AuthAPI --> AuthService
EntryAPI --> Utils
ReviewAPI --> DeepSeekService
AuthService --> PrismaClient
DeepSeekService --> PostgreSQL
```
...
### 开发工作流指南
基于增强的项目资料索引文档，推荐的开发工作流程包括：

1. **环境准备**
   - 安装 Node.js 18+ 和包管理器
   - 配置 PostgreSQL 数据库连接
   - 设置必要的 API Key（DeepSeek、邮件服务等）

2. **开发环境搭建**
   - 运行 `npm install` 安装依赖
   - 执行 `npm run db:deploy` 初始化数据库
   - 启动开发服务器 `npm run dev`

3. **日常开发流程**
   - 修改代码后自动热重载
   - 使用 ESLint 进行代码质量检查
   - 通过 Prisma Studio 管理数据库

4. **部署流程**
   - 构建生产版本 `npm run build`
   - 使用 PM2 或 Docker 进行部署
   - 配置环境变量和生产环境优化
