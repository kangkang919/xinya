...
## 更新摘要
**变更内容**   
- 更新了 PM2 进程管理配置章节，反映环境变量加载问题的修复
- 新增了 --env-file 标志的详细说明和使用指南
- 完善了生产环境部署的最佳实践建议
- 增强了容器化部署的环境变量注入说明
...
B --> I["环境变量文件<br/>.env.production"]
J["部署脚本<br/>deploy.sh"] --> B
```
...
**已更新** 环境变量注入方式现已支持多种方法，推荐使用显式文件路径方式以确保在生产环境中可靠加载。

环境变量注入方式
- **PM2 显式文件路径（推荐）**：在 ecosystem.config.js 中使用 `--env-file=/www/wwwroot/xinya/.env.production` 指定环境变量文件路径
- PM2 环境变量：在 ecosystem.config.js 的 env 中或通过 PM2 的 --env-file 指定 .env.production
...
### PM2 进程管理配置（ecosystem.config.js）
- name: 应用名称
- script: 指向 Node.js 可执行路径
- args: 启动参数，包含 `--env-file=/www/wwwroot/xinya/.env.production` 和 `node_modules/.bin/next start -p 3000`
- cwd: 工作目录 `/www/wwwroot/xinya`
- instances: 实例数（当前为 1）
- autorestart: 崩溃自动重启
- watch: 是否监听文件变更（生产应关闭）
- max_memory_restart: 内存上限触发重启阈值（512M）
- env: 注入环境变量（NODE_ENV=production）

**已更新** PM2 配置现已修复环境变量加载问题，通过显式使用 `--env-file` 标志指向 `/www/wwwroot/xinya/.env.production` 文件，解决了容器化或进程管理环境中的部署问题。
...
- **环境变量文件管理**：确保 `.env.production` 文件权限设置为 600，仅所有者可读，防止敏感信息泄露。
...
### 部署脚本与自动化
- 一键部署脚本
  - deploy.sh 提供完整的一键部署流程，包括依赖安装、数据库迁移、构建和 PM2 启动。
- 部署步骤
  - 安装依赖 → 生成 Prisma Client → 执行数据库迁移 → 构建项目 → 启动 PM2 进程
- 自动化特性
  - 自动删除旧进程、保存 PM2 配置、设置开机自启动
...
- **环境变量加载失败（新增）**
  - 检查 `.env.production` 文件是否存在且路径正确：`/www/wwwroot/xinya/.env.production`
  - 确认文件权限设置为 600，仅所有者可读
  - 查看 PM2 日志：`pm2 logs xinya` 确认环境变量是否正确加载
  - 验证 PM2 配置中的 `--env-file` 参数指向正确的绝对路径
...
## 结论
通过合理的运行时选择、严格的环境变量管理与 PM2 进程治理，结合 Next.js 生产优化与外部服务（数据库、邮件、AI）的健壮性设计，可以保障"心芽"在生产环境的稳定与高效运行。**特别需要注意的是，最新的 PM2 配置已通过显式使用 `--env-file` 标志解决了环境变量加载问题，确保了在生产环境中的可靠性。**建议在上线前完成压测与演练，完善监控告警与回滚预案。
...
- **环境变量文件路径：--env-file=/www/wwwroot/xinya/.env.production（已修复）**
- 日志轮转：建议启用 PM2 日志轮转（概念性建议）

**已更新** PM2 配置现已包含显式的环境变量文件路径，确保在生产环境中可靠加载所有必要的配置信息。
...
**已更新** 在容器化部署中，建议通过以下方式注入环境变量：
- 使用 `--env-file` 标志挂载环境变量文件
- 通过 docker-compose.yml 的 environment 字段注入
...
### 部署脚本使用指南（deploy.sh）
- 一键部署流程
  - 自动执行依赖安装、数据库迁移、项目构建和 PM2 启动
- 部署前准备
  - 确保服务器上有 Node.js、npm、PM2 环境
  - 准备好 `.env.production` 文件并放置在 `/www/wwwroot/xinya/` 目录下
- 部署后验证
  - 检查 PM2 进程状态：`pm2 status`
  - 查看应用日志：`pm2 logs xinya`
  - 测试应用访问：`curl http://localhost:3000`

章节来源
- [deploy.sh:1-37](file://deploy.sh#L1-L37)

### 环境变量文件管理最佳实践
- 文件位置：`/www/wwwroot/xinya/.env.production`
- 文件权限：设置为 600（仅所有者可读）
- 文件内容：包含所有生产环境所需的敏感配置
- 备份策略：定期备份到安全位置，如 `doc/.env.production.备份`
- 版本控制：`.env.production` 文件不应提交到版本控制系统

**已更新** 通过 PM2 的 `--env-file` 标志，环境变量文件可以在容器化和进程管理环境中可靠加载，解决了之前可能存在的加载问题。

章节来源
- [ecosystem.config.js:1-15](file://ecosystem.config.js#L1-L15)
- [doc/新电脑程序转移主人提醒.md:146-175](file://doc/新电脑程序转移主人提醒.md#L146-L175)