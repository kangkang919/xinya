...
| 功能点 | 描述 | 优先级 |
|---|---|---|
| F1.1 | 富文本编辑（Tiptap 引擎）：加粗、斜体、下划线、删除线、有序/无序列表、标题(H2)、引用块、代码块、分隔线、字体颜色（6色） | P0 |
| F1.2 | 标题必填，正文富文本 | P0 |
...
> 编辑器底层已从手工 contentEditable 迁移至 **Tiptap**（基于 ProseMirror），所有富文本功能由框架原生支持，光标管理、浏览器兼容性由框架内部处理。

| 功能 | HTML 输出 | 说明 | 状态 |
|---|---|---|---|
| 加粗 | `<strong>文字</strong>` | 行内加粗，快捷键 Ctrl+B | ✅ 已实现 |
| 斜体 | `<em>文字</em>` | 行内斜体，快捷键 Ctrl+I | ✅ 已实现 |
| 下划线 | `<u>文字</u>` | 行内下划线，快捷键 Ctrl+U | ✅ 已实现 |
| 删除线 | `<s>文字</s>` | 行内删除线 | ✅ 已实现 |
| 标题 | `<h2>标题</h2>` | 仅支持 H2 一层，不支持 H1/H3/H4；按钮切换 h2↔p | ✅ 已实现 |
...
| 分隔线 | `<hr>` | 输入 `---` 后按空格自动转为灰色横线；工具栏按钮亦可 | ✅ 已实现 |
| 字体颜色 | `<span style="color:色值">文字</span>` | 6 色：#333333 #8BC34A #42A5F5 #FF8C42 #795548 #e57373 | ✅ 已实现 |
| 换行 | `<br>` | 段内换行 | ✅ 已实现 |
| 段落 | `<p>` | Tiptap 标准段落标签 | ✅ 已实现 |

**旧内容兼容：** 数据库中的旧内容使用 `<div>` 段落和 `<font color>` 标签，编辑器加载时自动转换为 `<p>` 和 `<span style="color">`。查看页/分享页通过 `dangerouslySetInnerHTML` 渲染，新旧格式均正常显示。
...
| F6.3 | 数据导出：仅 Markdown 格式，含标题/正文/标签/创建时间，按钮位于版本更新上方；正文使用 turndown 将富文本 HTML 转为标准 Markdown 语法（加粗/列表/代码块等），确保任何 Markdown 软件均可正常渲染 ✅ | P0 |
...
| F11.2 | 搜索结果按相关度排序：置顶 > 标题匹配 > 内容匹配 > 时间倒序；使用 ILIKE 模糊匹配（PostgreSQL 原生支持，无需分词） | P0 |
...
#### F14: 豆苗学习助手（AI 聊天，2026-09-02 开发）

> 完整规格见《豆苗学习助手-PRD.md》；人设/边界/检索规则详见《豆苗学习助手-需求文档.md》。代码均带「删除豆苗时移除」注释，可整体下线（F14.6）。

| 功能点 | 描述 | 优先级 |
|---|---|---|
| F14.1 | 底部导航「豆苗」入口 + 首次 3 步入设向导（语气/指导/称呼 预设 + 可选自由描述，可跳过用默认值） | P1 |
| F14.2 | 单一会话聊天：以本人非草稿心得为唯一知识库；标签→标题→内容 三级检索；AI 回答 + UI 检索标签徽章；寒暄/安全词本地快判 | P1 |
| F14.3 | 记忆系统：兴趣/薄弱点（对话/答题/用户指定 三来源，答题口径为正确率 <60% 且 ≥3 次），清单可见、逐条可删 | P2 |
...
| F14.9 | 豆苗可干预拾遗出题优先级（2026-09-03 新增）：新建 QuizPriority 表（tag/mode/multiplier/until/active）；支持 insert（插队直到未答题全部答完）和 weight（权重倍数）两种模式；豆苗对话可查询当前出题规划/设置优先级；出题逻辑（review-scheduler.ts）集成优先级配置；新增 API `/api/review/priority`；**关键约束**：新增心得生成的题目必须进入出题列表，优先级只影响抽取顺序不影响题目池完整性；**2026-09-09 修复**：insert 模式改为独立查询路径（先查该标签到期题→再查未答题→降级通用），解决原逻辑先取 50 道通用候选再过滤导致 insert 标签题被旧题挤出候选池的缺陷 | P2 |
...
| 2026-09-16 | A+C 方案上线：LLM 查询改写（query-reformulate.ts）提取精确搜索短语优先检索，失败降级 jieba 关键词；解决多词概念（如"用户故事地图"）被拆分为单词导致检索失效的问题 | 已验收 |
| 2026-09-23 | 修复优先级配置取消失效：用户说"取消插队"时，priority-intercept.ts 原 NEGATIVE_WORDS 正则直接返回不拦截，导致 LLM 回复"已取消"但数据库未实际更新；修复：新增 CANCEL_WORDS 检测取消意图，返回 mode="cancel"，chat.ts 中实际将对应标签优先级设为 active=false；新增 4 个取消意图测试用例，全量 108 测试通过 | 已验收 |
| 2026-09-23 | 修复新对话首条消息取消不生效 + LLM 幻觉调用接口：①取消拦截原要求历史上下文含优先级讨论，新对话首条说"取消AI安全的插队"不命中→消息发给 LLM；修复：取消意图改为当前消息同时含取消词+优先级话题词（插队/权重/优先级/出题）+标签名即可拦截，无需历史上下文；②system prompt 原指示 LLM"调用 GET/POST /api/review/priority"，但 LLM 无调用接口能力，导致回复"稍等，我调用接口"后无下文；修复：prompt 改为配置已注入直接引用+新增规则 13【禁止假装行动】；新增 4 个测试用例，全量 112 测试通过 | 已验收 |
...
| 2026-07-28 | F9 拾遗数据安全修复：重生成题目时仅删除未答记录，保留已答历史；新增附录A-0规则7/8/9（时区/删除安全/Schema变更） | 已验收 |
...
| 2026-08-13 | F2.14 分享页子标签排序修复：GET /api/share/[token] 查询标签时漏加 orderBy，导致访客视图子标签按数据库插入顺序返回而非用户手动排序顺序；修复：与 /api/tags 对齐，补 orderBy: [{ sortOrder: "desc" }, { name: "asc" }]，前端无需改动 | 已验收 |
| 2026-08-15 | F9.22 题目重生机制修复：发现原逻辑"已有待答题目则跳过"过于激进，导致所有有答对记录的心得（13篇）全部被跳过，重生从未实际执行；修复：移除跳过逻辑，改为每次定时执行时为所有答对过的心得生成新题（旧题退休、新题入库），调度器本身有 lastCardDate 防重复机制无需额外跳过；约束文档新增"完整出题与调度逻辑"章节（题目来源4触发点、调度器5级优先级、答题后间隔计算、重生流程5步） | 已验收 |
...
| 2026-08-24 | F6.3 导出 Markdown 修复：原导出直接拼接数据库 HTML 内容，导致部分心得在 Markdown 预览器中原样显示 `<div>`/`<b>` 等标签；新增 turndown 库将 HTML 转为标准 Markdown 语法（加粗/列表/代码块等），所有心得导出后在任何 Markdown 软件中均可正常渲染排版；涉及文件 lib/export-utils.ts（新增 getTurndownService 懒加载 + emptyDiv 规则清理空标签），新增依赖 turndown + @types/turndown | 已验收 |
| 2026-08-24 | F6.3 导出 Markdown 二次修复：段落间距丢失修复（编辑器用 `<div>` 分段，turndown 默认不加空行，导致段落合并成一；修改 div 规则前后加 `\n\n`）；②等号转义修复（turndown 把 `=` 当 Markdown 特殊字符转义为 `\=`，后处理替换回 `=`）；③多余星号修复（嵌套 `<b>` 标签导致 `****总纲**`，后处理清理为 `**总纲**`）；涉及文件 lib/export-utils.ts（修改 div 规则 + 新增 cleanMarkdown 后处理函数） | 已验收 |
| 2026-08-24 | 编辑器粘贴行为修复：原 handlePaste 逻辑检查光标是否在 `<p>` 标签内，但编辑器默认用 `<div>` 包裹段落，条件不满足导致创建新 `<p>` 标签产生前后换行；修复为直接用 `document.execCommand('insertHTML')` 在光标位置插入纯文本，不创建新块；涉及文件 components/Editor.tsx | 已验收 |
| 2026-08-24 | 编辑器粘贴二次修复：`insertHTML` 会改变 DOM 结构和光标位置，导致代码块按钮插入位置错位（出现在文章底部）；改用 `insertText` 插入纯文本，浏览器自动处理换行和光标位置，不影响后续操作；涉及文件 components/Editor.tsx | 已验收 |
...
| 2026-08-26 | 富文本样式统一：将编辑器/查看页/分享面板/分享链接页四处重复的富文本 CSS 样式抽取为共享模块 `lib/rich-text-styles.ts`，消除重复和遗漏；新增 `p:empty { min-height: 1.625em }` 确保空段落高度不塌缩，解决编辑器空行保存后消失的问题；涉及文件 lib/rich-text-styles.ts（新建）、components/Editor.tsx、app/entry/[id]/view/page.tsx、components/SharePanel.tsx、app/share/[token]/page.tsx | 已验收 |
