...
| 2026-09-23 | 修复优先级配置取消失效：用户说"取消插队"时，priority-intercept.ts 原 NEGATIVE_WORDS 正则直接返回不拦截，导致 LLM 回复"已取消"但数据库未实际更新；修复：新增 CANCEL_WORDS 检测取消意图，返回 mode="cancel"，chat.ts 中实际将对应标签优先级设为 active=false；新增 4 个取消意图测试用例，全量 108 测试通过 | 待验收 |
| 2026-09-23 | 修复新对话首条消息取消不生效 + LLM 幻觉调用接口：①取消拦截原要求历史上下文含优先级讨论，新对话首条说"取消AI安全的插队"不命中→消息发给 LLM；修复：取消意图改为当前消息同时含取消词+优先级话题词（插队/权重/优先级/出题）+标签名即可拦截，无需历史上下文；②system prompt 原指示 LLM"调用 GET/POST /api/review/priority"，但 LLM 无调用接口能力，导致回复"稍等，我调用接口"后无下文；修复：prompt 改为配置已注入直接引用+新增规则 13【禁止假装行动】；新增 4 个测试用例，全量 112 测试通过 | 待验收 |
