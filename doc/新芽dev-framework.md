...
| 2026-08-26 | 富文本样式统一：将编辑器/查看页/分享面板/分享链接页四处重复的富文本 CSS 样式抽取为共享模块 `lib/rich-text-styles.ts`，消除重复和遗漏；新增 `p:empty { min-height: 1.625em }` 确保空段落高度不塌缩，解决编辑器空行保存后消失的问题；涉及文件 lib/rich-text-styles.ts（新建）、components/Editor.tsx、app/entry/[id]/view/page.tsx、components/SharePanel.tsx、app/share/[token]/page.tsx | 待验收 |
