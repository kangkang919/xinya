...
| 日期 | 变更内容 | 状态 |
| :--- | :--- | :--- |
| 2026-09-09 | 修复 insert 优先级候选池挤压缺陷：原逻辑先取 50 道通用到期题再过滤 insert 标签，当旧题超过 50 道时 insert 标签题被挤出候选池导致优先级完全失效；改为 insert 标签独立查询路径（先查该标签到期题→再查未答题→降级通用），不受 50 道限制 | 待验收 |
